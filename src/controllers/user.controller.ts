...
export class UserController {
  ...
  async getPostsForUser(req: Request, res: Response, next: NextFunction): Promise<void> {
    try {
      ...
      const { data, total } = await this.postRepository.findManyByAuthorId(parentId, params);
      res.json(buildPaginatedResponse(data, total, params));
    } catch (err) {
      next(err);
    }
  ...
  async createPostForUser(req: Request, res: Response, next: NextFunction): Promise<void> {
    try {
      ...
      const record = await this.postRepository.create({ ...data, authorId: parentId });
      res.status(201).json(record);
    } catch (err) {
      next(err);
    }
  ...
  async getCommentsForUser(req: Request, res: Response, next: NextFunction): Promise<void> {
    try {
      ...
      const params = parseListQuery(req, nestedFilterFields, nestedSortFields);
      const { data, total } = await this.commentRepository.findManyByAuthorId(parentId, params);
      res.json(buildPaginatedResponse(data, total, params));
    } catch (err) {
      next(err);
    }
  ...
  async createCommentForUser(req: Request, res: Response, next: NextFunction): Promise<void> {
    try {
      // Parent ID comes from the authenticated user's token, not the URL
      const parentId = req.user!.id;
      const data = validateCommentCreate(req.body);
      // Inject the parent FK; ignore any authorId provided in the body
      const record = await this.commentRepository.create({ ...data, authorId: parentId });
      res.status(201).json(record);
    } catch (err) {
      next(err);
    }
