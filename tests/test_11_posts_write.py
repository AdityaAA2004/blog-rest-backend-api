...
def run(ctx: TestContext) -> None:
    ...
    # 11-val-title  missing title → 400
    if token1:
        resp = ctx.req("POST", _create_path, token=token1,
                       body={"content": "This is valid post content"})
        ctx.assert_status(resp, 400, "POST post missing title → 400")

    # 11-val-content  missing content → 400
    if token1:
        resp = ctx.req("POST", _create_path, token=token1,
                       body={"title": "Valid Post Title"})
        ctx.assert_status(resp, 400, "POST post missing content → 400")

    # 11-spoof  ownership spoofing
    if token1:
        resp = ctx.req("POST", _create_path, token=token1,
                       body={"title": "Spoof Test", "content": "Spoofing attempt body text", "authorId": user2_id})
